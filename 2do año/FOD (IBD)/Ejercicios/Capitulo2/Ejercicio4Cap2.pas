...
procedure actualizar (var archprod:arch_prod);
  var
    prod:productos;
  begin
    reset(archprod);
    while not eof(archprod) do begin
      read(archprod,prod);
      if (prod.tipo = 'comestible') then begin
        prod.precio:= (prod.precio * 1.05);
        seek(archprod,filepos(archprod) - 1);
        write(archprod,prod);
        end
      else
        if (prod.tipo = 'limpieza') then begin
          prod.precio := (prod.precio * 1.1);
          seek(archprod,filepos(archprod) - 1);
          write(archprod,prod);
          end
        else
          prod.precio := (prod.precio * 1.2);
          seek(archprod,filepos(archprod) - 1);
          write(archprod,prod);
      end;
    close(archprod);
